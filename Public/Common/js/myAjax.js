...
function changeTableVal(table,id_name,id_value,field,obj)
{
		var src = "";
		 ...
		 {          
				src = '/Public/Admin/img/yes.png';
				var value = 1;
				
		 }else{                    
				src = '/Public/Admin/img/cancel.png';
				var value = 0;
		 }                                                 
		$.ajax({
				url:"/index.php?m=Admin&c=Ad&a=changeTableVal&table="+table+"&id_name="+id_name+"&id_value="+id_value+"&field="+field+'&value='+value,			
...
// 修改指定表的排序字段
function updateSort(table,key_name,key_value,field_name,obj)
{		       
	var value = $(obj).val();
	$.ajax({
			url:"/index.php?m=Admin&c=Ad&a=changeAdField&table="+table+"&"+key_name+"="+key_value+"&"+field_name+"="+value,
			success: function(data){									

...
(function(w, t) {
	function address() {
		...
		this.submitOrder = function(formId) {

			var obj = $('.payment .active');
			
			var pTypeId = obj.attr('value');
			
			$("#payType").val(pTypeId);
			
			var value = $('#addressId').val();
			if (!value || value == 0) {
			...
			if (!$('#express').val()) {
				toastr.error('请选择配送方式');
				return false;
			}

			if (!$("#payType").val()) {
				toastr.error('请选择支付类型');
				return false;
			}
			
			if (pTypeId == PAY_ID) {
				var money = $('#priceMonery').val();
				if (balanceQ < money) {
					toastr.error('余额不足');
					return false;
				}
            ...
            if (!checkSubmitFlg) {
                ...
                checkSubmitFlg = true;
                var check = $("#check").val()
                $.get(CHECK,{check:check}, function (res) {
                    if(res.status == 1){

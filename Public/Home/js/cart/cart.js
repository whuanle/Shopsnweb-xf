    ...
    return showDialog(2, function() {
        ...
        $.ajax({
            ...
            success: function(ret) {
                ...
                var cart_id   = top.attr('data-id');
                var goods_id  = top.attr('data-goods-id');
                var price     = top.find('.p-price b').html();
                var title     = top.find('.p-goods .p-name a').html();
                var goods_num = top.find('.p-quantity input').val();
                var data      = {
                    'cart_id':   cart_id,
                    'goods_id':  goods_id,
                    'goods_num': goods_num,
...
function update_input_number(url, cart_id, obj, type) {
    ...
    if (type == 1) { // 普通商品
        ...
        html.each(function (index, ele) {
            var price = $(ele).find('div.p-price b').text();
            price = parseFloat(price.substring(1));
            price = isNaN(price) ? 0 : price;
            $(ele).find('div.p-quantity').text(N);
            $(ele).find('div.p-sum em').text((N * price).toFixed(2))
        });
    }
    sum();
...
// 数量减,套餐商品
$('.cart-main .cart-list .search-main .decrement').on('click', function () {
    var N = $('.cart-main .cart-list .search-main input').eq($(this).parent().parent().index()).val();
    N = isNaN(parseInt(N)) ? 0 : N;
    if(N <= 1){
        $('.cart-main .cart-list .search-main .decrement').eq($(this).parent().parent().index()).addClass('active');
    }else{
        N--;
        $('.cart-main .cart-list .search-main input').eq($(this).parent().parent().index()).val(N);
...
// 数量加,套餐商品
$('.cart-main .cart-list .search-main .increment').on('click',function(){
    var N = $('.cart-main .cart-list .search-main input').eq($(this).parent().parent().index()).val();
    N = isNaN(parseInt(N)) ? 0 : N;
    N++;
    $('.cart-main .cart-list .search-main input').eq($(this).parent().parent().index()).val(N);
    if(N >= 1){
        $('.cart-main .cart-list .search-main .decrement').eq($(this).parent().parent().index()).removeClass('active');

...
<link rel="stylesheet" href="http://www.shopsn.xyz/Public/Home/css/poop.css" />
<div class="poopcer-content-wrap">
	...
	<?php if(!empty($goodsData)): ?><div class="popc-centent-wrap w">
	...
	<div class="time-centent-main w">
		<a href="javascript:;" class="btn-fl"></a> <a href="javascript:;"
			class="btn-fr"></a>
		<ul class="clearfix">
			<?php if(is_array($goodsData)): foreach($goodsData as $key=>$value): ?><li class="fl">
				<div class="guild">
					<span class="fl one">限时</span> <span class="fl">抢购</span>
				</div>
				<div class="discount">
					<div class="span">
						<?php $total = number_format($value[$poopModel::$expression_d],1) ?>
						<p><?php echo ($total); ?>/折</p>
						<!--<p><?php echo ($value[$type::$promationName_d]); ?></p>-->
					</div>
				</div> <!--缩略图-->

				<div class="screenshot">
					<a href="<?php echo U('Goods/poopByGoodsDetail', ['id' => $value[$poopModel::$goodsId_d]]);?>"><img
						src="<?php echo ($value[$goodsImages::$picUrl_d]); ?>"></a>
				</div>
				<div class="products-description">
					<p><?php echo ($value[$goodsModel::$title_d]); ?></p>
					<?php $status = $value['poopStatus']; $price = $value[$goodsModel::$priceMember_d]; $price=($price<0.01)?0.01:$price; $price=sprintf("%.2f", $price); ?>
					...
					<div class="products-prices">
						...
						<?php switch($status): case "0": $new_price=$price*$value[$poopModel::$expression_d]/10 ;$new_price=($new_price<0.01)?0.01:$new_price;$new_price=sprintf("%.2f", $new_price); echo ($new_price); break;?>
							<?php case "1": $new_price1=$price-$value[$poopModel::$expression_d] ;$new_price1=($new_price1<0.01)?0.01:$new_price1; $new_price1=sprintf("%.2f", $new_price1); echo ($new_price1); break;?> <?php case "2": case "-1": echo ($price); break; endswitch;?>
...
<script>
	var sec = <?php echo ($format); ?>;</script>
	<script src="http://www.shopsn.xyz/Public/Home/js/expansion.js"></script>
	<script src="http://www.shopsn.xyz/Public/Home/js/common/common.js"></script>
	<script src="http://www.shopsn.xyz/Public/Home/js/poop/poop.js"></script>
	<script src="http://www.shopsn.xyz/Public/Home/js/navbannerurl.js"></script>
	<script type="text/javascript">dataObj.getData("<?php echo U('ajaxGetPoopGoods');?>", 'addHtml');</script>

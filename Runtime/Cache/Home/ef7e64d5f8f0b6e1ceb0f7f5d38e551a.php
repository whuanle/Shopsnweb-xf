...
<script>
    ...
    <dd>
		...
		<!--内容-->
		<div class="mordrMain fr intgl-content-wrap">
			<div class="title clearfix">
				<ul class="clearfix fl nav">
					<li class="fl <?php if($type == '1'): ?>hover<?php endif; ?>"><a href="<?php echo U('Assets/integral');?>">积分明细</a></li>
					<li class="fl <?php if($type == '2'): ?>hover<?php endif; ?>"><a href="<?php echo U('Assets/integral_use');?>">积分收入</a></li>
					<li class="fl <?php if($type == '3'): ?>hover<?php endif; ?>active"><a href="<?php echo U('Assets/integral_expen');?>">积分支出</a></li>
				</ul>
			</div>
			<div class="content-main">
				<div class="data-wrap">
					<ul class="data clearfix">
						<li class="fl right">
							<h2>可用的积分</h2>
							<h3><?php echo ($integral["integral"]); ?></h3>
						</li>
						<li class="fl center">
							<h2>将要过期的积分</h2>
							<h3><?php echo ($integral["overdue_integral"]); ?></h3>
							<p>(将于<?php echo (date("Y.m.d",$integral["overdue"])); ?>过期)</p>
						</li>
						<li class="fl left">
							<a href="<?php echo U('Assets/punkte');?>" class="btn">兑换超值商品</a>
						</li>
					</ul>
				</div>
				<dl class="content">
					<dt class="clearfix">
						<span class="fl angaben">来源/用途</span>
						<span class="fl variable">积分变化</span>
						<span class="fl timer">日期</span>
						<span class="fl anm">备注</span>
					</dt>
					<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd class="clearfix">
							...
							<div class="products-description clearfix fl">
								<div class="fl screenshot">
									<img src="<?php echo ($vo["images"]); ?>">
								</div>
								<div class="fl left-main">
									<p><?php echo ($vo["title"]); ?></p>
									<span>编号：<?php echo ($vo["goods_id"]); ?></span>
								</div>
							</div>
                            <?php if($vo["type"] == 1): ?><div class="variable fl">+<?php echo ($vo["integral"]); ?></div>
							<?php else: ?>
								<div class="variable fl">-<?php echo ($vo["integral"]); ?></div><?php endif; ?>
							<div class="timer fl"><?php echo (date("Y年m月d日 H:i:s",$vo["trading_time"])); ?></div>
							<div class="anm fl"><?php echo ($vo["remarks"]); ?></div>

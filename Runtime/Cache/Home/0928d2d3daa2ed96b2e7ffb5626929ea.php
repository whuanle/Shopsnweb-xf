    ...
    <dd>
		...
		<div class="mordrMain fr reqoer-content-wrap">
			...
			<div style="width:100%;height:30px;margin-top: 20px;margin-bottom: 20px;">
				<form action="<?php echo U('GoodsCenter/search');?>" method="post" onsubmit="return check()">
					<div style="border:1px solid #999;width:230px;height:28px;position: relative;float: left;">
						<input type="text" name="title" id="title" style="width:160px;height:28px;border: none;outline: none;text-indent: 15px;"placeholder="商品名称">
					</div>
			...
			<input type="hidden" id="p-value" value="<?php echo ($id); ?>"/>
				...
				<div class="already">
					<dl class="content">
						...
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd class="clearfix">
								<span class="fl look">
									<?php if(empty($vo["brand"])): ?>&nbsp;
									<?php else: ?>
									<?php echo ($vo["brand"]); endif; ?>
								</span>
								<span class="fl look"><?php echo ($vo["id"]); ?></span>
								<span class="fl look">
								    <?php if(empty($vo["code"])): ?>&nbsp;
									<?php else: ?>
									<?php echo ($vo["code"]); endif; ?></span>
								<a href="javascript:;" title="<?php echo ($vo["title"]); ?>"><span class="fl name"><?php echo (substr($vo["title"],0,60)); ?></span></a>
								<span class="fl look"><?php echo ($vo["price_market"]); ?></span>
								<span class="fl look"><?php echo ($vo["price_member"]); ?></span>
...
<script>
	...
	// 	var id = $('#p-value').val();
	// 	var url = '<?php echo U("GoodsCenter/expGoods");?>';

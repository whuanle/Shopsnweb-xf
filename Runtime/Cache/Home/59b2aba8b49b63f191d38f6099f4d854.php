	...
	<!--内容-->
		<div class="mordrMain fr">
			...
			<div class="balan-title clearfix">
				<span class="fl">可用余额：<em>￥<?php echo ($balance["account_balance"]); ?></em></span>
				<span class="fl">锁定余额：<em>￥<?php echo ($balance["lock_balance"]); ?></em></span>
				<span class="fl">账户状态：<em>
											    <?php switch($balance["status"]): case "1": ?>有效<?php break;?>
											   		<?php case "2": ?>已过期<?php break;?>
											   		<?php case "3": ?>有效<?php break;?>
											   		<?php case "4": ?>有效<?php break; endswitch;?>	
				                           </em></span>
				<span class="fl"><a href="javascript:;">安全中心</a>保护账户资产安全</span>
				<a href="<?php echo U('Assets/recharge');?>" class="fl recharge">充值</a>
			</div>
			<!--收支明细-->
			<div class="balance-detailed">
				<ul class="nav clearfix">
				    <?php if($status == '1'): ?><li class="fl active">近三个月收支明细</li>
						<li class="fl" id="aa">三个月前收支明细</li>
					<?php else: ?>
						<li class="fl">近三个月收支明细</li>
						<li class="fl active" id="aa">三个月前收支明细</li><?php endif; ?>
				</ul>
				<dl>
					<dt class="clearfix">
					...
					<?php if(empty($data["res"])): ?><dd align="center" style="color:red;margin-top:10px;">暂时没有记录!</dd>
					<?php else: ?>
						<?php if(is_array($data["res"])): $i = 0; $__LIST__ = $data["res"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd class="clearfix">
								<span class="fl timer"><?php echo ($vo["recharge_time"]); ?></span>
								<span class="fl deposit">￥<?php echo ($vo["deposit"]); ?></span>
								<span class="fl expenditure">￥<?php echo ($vo["pay"]); ?></span>
								<span class="fl remarks"><?php echo ($vo["description"]); ?></span>
							</dd><?php endforeach; endif; else: echo "" ;endif; endif; ?>
				</dl>
...
<script>
	...
	aLi.on('click',function(){
		...
		N = $(this).index()+1;
		$.ajax({
			url : '<?php echo U("Assets/balance_front");?>',
			dataType : "json",
			type : 'get',
			data : {flag:N},

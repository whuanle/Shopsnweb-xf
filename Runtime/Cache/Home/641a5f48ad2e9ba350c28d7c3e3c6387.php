		...
		<div class="mordrMain fr atest-content-wrap">
			...
			<dl class="con-main">
				...
				<?php if(is_array($data["res"])): $i = 0; $__LIST__ = $data["res"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd class="clearfix">
						...
						<span class="fl order-nr">
						    <?php if(empty($vo["order_id"])): ?>&nbsp;
						    <?php else: ?>
							<?php echo ($vo["order_id"]); endif; ?>
						</span>
						<span class="fl sales-reason">
							<?php if($vo["type"] == 3): ?>维修
							<?php else: ?>
							    <?php echo ($vo["tuihuo_case"]); endif; ?>							
						</span>
						<span class="fl state">
							<?php switch($vo["status"]): case "0": ?>审核中<?php break;?>
								...
								<?php case "6": ?>已撤销<?php break; endswitch;?>	
						</span>
						<span class="fl timer"><?php echo (date("Y-m-d H:i",$vo["create_time"])); ?></span>
						<a href="<?php echo U('Service/after_sale_details',array('id'=>$vo['id']));?>" class="fl operation">查看详情 </a>

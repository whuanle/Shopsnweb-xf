...
<script>
    ...
    <dd>
		...
		<div class="mordrMain fr">
			...
			<!--page-->
			<!--订单-->
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="myderCentent">
					...
					<div class="con clearfix">
							...
							<?php if(is_array($vo["goods"])): $i = 0; $__LIST__ = $vo["goods"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?><div class="conLoop">
									<div class="one clearfix fl">
										...
										<div class="conRight fl">
											<a href="<?php echo U('Goods/goodsDetails',array('id'=>$goods['id']));?>" class="hover"><?php echo ($goods["title"]); ?></a>
											<p>商品编号：<?php echo ($goods["id"]); ?></p>
											<p>采购单位：中箱（20）</p>
										</div>
									</div>
						...
						<div class="fr clearfix conFr">
							...
							<div class="five fl details">
								<a href="<?php echo U('Order/buy_again_order',array('id'=>$vo['order_id']));?>" class="hover">再次购买</a>
							</div>
						</div>
					</div>
				</div><?php endforeach; endif; else: echo "" ;endif; ?>

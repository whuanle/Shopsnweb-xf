    ...
    <dd>
		...
		<div class="toBeted fr">
			<ul class="title clearfix">
				<li class="fl active"><a href="javascript:;">待评价订单<em>（<?php if(empty($count)): ?>0<?php else: ?> <?php echo ($count); endif; ?>）</em></a></li>
				<li class="fl"><a href="javascript:;">待晒单<em>（<?php if(empty($w_count)): ?>0<?php else: ?> <?php echo ($w_count); endif; ?>）</em></a></li>
				...
				<div class="centent-main">
				    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dl>
							<dt>
								...
								<span>订单号：<?php echo ($vo["order_id"]); ?></span>
							</dt>
							<dd class="clearfix">
								<div class="fl conFl clearfix">
								    <?php if(is_array($vo["goods"])): $i = 0; $__LIST__ = $vo["goods"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?><div class="loop">
											...
											<div class="fr numberFr">x<?php echo ($goods["goods_num"]); ?>
                                            <?php if($goods["comment"] == '1'): ?><a href="javaScript:;" class="fr btn">已评价</a>
                                            <?php else: ?>
												<a href="<?php echo U('Order/comment',array('goods_id'=>$goods['goods_id'],'order_id'=>$vo['order_id']));?>" class="fr btn">评价</a><?php endif; ?>
											</div>
										</div><?php endforeach; endif; else: echo "" ;endif; ?>
								</div>
								<div class="fr conFr">
									<span class="fl name"><?php echo ($vo["realname"]); ?></span>
									<span class="fl payment">支付金额￥<?php echo ($vo["price_sum"]); ?></span>
									<span class="fl evaluate">
										<a href="<?php echo U('Order/order_details',array('id'=>$vo['order_id']));?>">订单详情</a><br>										
									</span>
								</div>
							</dd>
			...
			<div class="centent-wrap">
				...
				<?php if(is_array($Waiting_goods)): $i = 0; $__LIST__ = $Waiting_goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$w_goods): $mod = ($i % 2 );++$i;?><div class="products-parent">
						<div class="products clearfix">
							<div class="fl conFl">
								<div class="screenshot fl">
									<a href="javascript:;"><img src="<?php echo ($w_goods["images"]); ?>"></a>
								</div>
								<div class="products-description fl">
									<p><a href="javascript:;"><?php echo ($w_goods["title"]); ?></a></p>
									<span>购买时间：<?php echo (date("Y-m-d",$w_goods["create_time"])); ?></span>
								</div>
							</div>
							<div class="fr conFr">
								<a href="javascript:;" data-value="<?php echo ($w_goods["id"]); ?>">继续晒单</a>
							</div>
							<span class="triangle"></span>
						</div>
					</div>				
					<div class="products-review-main" id="review<?php echo ($w_goods["id"]); ?>" style="display:none;">
						<div class="experience clearfix">
							<span class="fl"><b>*</b>心得：</span>
							<div class="fr keyword clearfix">
							    <?php if(is_array($w_goods["feel"])): $i = 0; $__LIST__ = $w_goods["feel"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$feel): $mod = ($i % 2 );++$i;?><em class="fl"><?php echo ($feel["title"]); ?></em><?php endforeach; endif; else: echo "" ;endif; ?>
							</div>
						</div>
						<div class="experience clearfix">
							<span class="fl"><b>*</b>评论内容：</span>
							<div class="fr keyword clearfix">
							    <?php echo ($w_goods["content"]); ?>
							</div>
						</div>
						<form action="<?php echo U('Assets/uploadImage');?>" enctype="multipart/form-data" method="post" >
							...
							<div class="publish">
							    <input type="hidden" name="id" value="<?php echo ($w_goods["id"]); ?>">
			...
			<div class="centent-wrap">
				...
				<?php if(is_array($Alreadygoods)): $i = 0; $__LIST__ = $Alreadygoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$a_goods): $mod = ($i % 2 );++$i;?><ul class="centent-main">
						<li class="clearfix">
							<div class="products-description fl clearfix">
								<div class="fl screenshot">
									<a href="javascript:;"><img src="<?php echo ($a_goods["images"]); ?>"></a>
								</div>
								<a href="javascript:;" class="details fl"><?php echo ($a_goods["title"]); ?></a>
							</div>
							<div class="fl data"><?php echo (date("Y-m-d H:i:s",$a_goods["create_time"])); ?> 初评</div>
							<div class="fl state"><a href="<?php echo U('Assets/comment_check',array('id'=>$a_goods['id']));?>">查看评论</a></div>
						</li>
...
<!-- <script src="js/header.js"></script>
 -->
 <script src="http://www.shopsn.cn/Public/Home/js/toBeEvaluated.js"></script>

...
<script>
	...
	<div class="person-section clearfix">
		...
		<!--内容-->
		<div class="mordrMain fr opini-content-wrap">
			<div class="title clearfix">
				<ul class="clearfix fl nav">
					<li class="fl active hover"><a href="javascript:;">意见建议</a></li>
				</ul>
			</div>
			<div class="content-main">
				<div class="nav clearfix">
					<span class="fl left-main">选择订单</span>
					<!-- <span class="fr right-main">
						<input type="text" placeholder="商品名称、商品编号、订单编号" id="search" class="txt"><input type="button" value="查询" class="btn">
					</span> -->
				</div>
				<dl class="content">
					<dt class="clearfix">
						<span class="fl numbre">订单编号</span>
						<span class="fl comm">订单商品</span>
						<span class="fl timer">下单时间</span>
					...
					<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd class="clearfix">
							<div class="fl numbre"><?php echo ($vo["order_id"]); ?></div>
							<div class="fl comm clearfix">
								<a href="<?php echo U('Goods/goodsDetails',array('id'=>$vo['goods_id']));?>">
									<div class="screenshot fl"><img src="http://www.shopsn.cn/<?php echo ($vo["images"]); ?>"></div>
									<div class="products-description fr">
							...
							<div class="fl timer">
								<?php echo (date("Y-m-d",$vo["create_time"])); ?><br>
								<?php echo (date("H:i:s",$vo["create_time"])); ?>
							</div>
							<div class="oper fl"><a <?php if($userId['user_name']==null): ?>href='<?php echo U('public/login');?>'<?php else: ?>href='javascript:;' onclick='easemobim.bind({
        //请使用自己的租户ID
         tenantId: "39449",
         //是否隐藏小的悬浮按钮
         hide: true
        })'<?php endif; ?>>联系卖家</a></div>

...
<script>
	...
	<div class="myTck-content-wrap w">
		<div class="nav-wrap clearfix">
			<ul class="nav clearfix fl">
				<li class="fl btn1">
				    <?php if(empty($class_name)): ?><span>全部类目</span>
					<?php else: ?>
						<span><?php echo ($class_name["class_name"]); ?></span><?php endif; ?>
					<div class="dorpmenu">
						<ul>
						    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Assets/myTracks_class',array('class_id'=>$vo['id']));?>"><li class="option"><?php echo ($vo["class_name"]); ?></li></a><?php endforeach; endif; else: echo "" ;endif; ?>
						</ul>
					</div>
				</li>
			</ul>
		...
		<ul class="con-main clearfix">
		    <?php if(is_array($goods)): $i = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="fl" id="del<?php echo ($vo["id"]); ?>">
					<div class="screenshot">
						<a href="javascript:;"><img src="<?php echo ($vo["images"]); ?>"></a>
					</div>
					<div class="products-description">
						<p><a href="javascript:;"><?php echo (substr($vo["title"],0,80)); ?></a></p>
						<span>¥ <?php echo ($vo["price_member"]); ?></span>
					</div>
					<div class="products-operation clearfix">
						<a href="<?php echo U('Product/ProductList',array('cid'=>$vo['class_id']));?>" class="fl find">找相似</a>		
						<input type="button" value="删除" class="fl del" data-value="<?php echo ($vo["id"]); ?>">
					</div>

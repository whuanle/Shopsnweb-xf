                 ...
                 <div class="box-header">
                    <nav class="navbar navbar-default">      
                        <div class="collapse navbar-collapse">
                          <form class="navbar-form form-inline" action="<?php echo U('Combo/index');?>" method="post">
                            <div class="form-group">
                                <input type="text" name="goods_id" class="form-control" placeholder="商品ID" value="<?php echo ($goods_id); ?>">
                            </div>
                            <button type="submit" class="btn btn-primary">查询</button>
                            <div class="form-group pull-right">
                                <a href="<?php echo U('Combo/accHandle', ['act'=>'view']);?>" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> 新增推荐配件</a>
                            </div>                
                          </form>
                        </div>
                    </nav>
                      ...
                      <table id="list-table" class="table table-bordered table-striped dataTable">
                        ...
                        <tbody>
                          <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr role="row">    
                             ...
                             <td>
                                 <img width="20" height="20" src="/Public/Admin/img/<?php if($vo[status] == 1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('goodsAccessories','id','<?php echo ($vo["id"]); ?>','status',this)"/>
                             </td>
                             <td>
                              <a class="btn btn-primary" href="<?php echo U('Combo/accHandle',array('act'=>'view','id'=>$vo['id']));?>"><i class="fa fa-pencil"></i></a>
                              <a class="btn btn-danger" onclick="delfunc(this)" data-url="<?php echo U('Combo/accHandle');?>" data-id="<?php echo ($vo['id']); ?>"><i class="fa fa-trash-o"></i></a>
                             </td>
